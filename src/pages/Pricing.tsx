...
const Pricing = () => {
  ...
  const handleSubscribe = async () => {
    ...
    if (!user) {
      ...
      navigate(`${localizePath('/auth', lang)}?next=${encodeURIComponent(`${pathname}${search}`)}`);
      return;
    }
    ...
    await openCheckout({
      interval,
      locale: lang,
      returnPath: localizePath('/pricing', lang),
    });
  };
  ...
  const openPortal = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('customer-portal', {
        body: { locale: lang, returnPath: localizePath('/pricing', lang) },
      });

      if (error || !data?.url) throw new Error('portal_failed');
        ...
        <div className="grid lg:grid-cols-2 gap-6 items-stretch">
          <div className="glass rounded-3xl p-10 flex flex-col">
            ...
            <Button asChild variant="outline" className="rounded-xl border-border/60 hover:bg-secondary/40">
              <LLink to="/#advisor">
                {t('pages.pricing.free_cta')} <ArrowRight className="w-4 h-4 ml-1" />
              </LLink>
            </Button>
          </div>
            ...
            {isActive ? (
              <div className="space-y-3">
                <div className="text-sm text-accent font-medium text-center">
                  ...
                  {subscription?.subscription_status === 'canceled' && ` ${t('pages.pricing.cancels_at_end')}`}
                </div>
