...
const CollectionDetail = () => {
  ...
  const description = l(c.description);
  const body = l(c.body);

  return (
    <PageShell>
      <SEO
        title={`${title} · AUTOVERE`}
        description={`${description} ${body.slice(0, 100)}`}
        image={c.image}
        type="article"
      />
      <section className="relative h-[70vh] min-h-[500px] overflow-hidden">
        ...
        <div className="container relative z-10 h-full flex flex-col justify-end pb-16 max-w-3xl">
          ...
          <h1 className="text-5xl md:text-7xl font-bold tracking-tighter leading-[1.05]">{title}</h1>
          <p className="text-lg md:text-xl text-muted-foreground mt-6">{description}</p>
        </div>
      </section>
