...
const Studio = () => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (res.ok) {
        toast.success(`Generated ${json.inserted_count} new suggestions`);
      ...
      <PageShell>
        <section className="container max-w-md py-32">
          <div className="glass rounded-3xl p-10">
            ...
            <h1 className="text-2xl font-semibold tracking-tight mb-2">AutoVere Studio</h1>
            <p className="text-sm text-muted-foreground mb-8">
              Internal AutoVere editorial control. Authentication required.
            </p>
    ...
    <PageShell>
      <section className="container max-w-5xl py-16">
        <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
          <div>
            <div className="text-xs uppercase tracking-[0.3em] text-accent mb-3">Studio</div>
            <h1 className="text-4xl md:text-5xl font-bold tracking-tighter">
              Library expansion queue
            </h1>
            <p className="text-muted-foreground mt-3 max-w-xl">
              AI proposes vehicles that fit AutoVere's voice. You approve what enters the library.
            </p>
          </div>
        ...
        <div className="space-y-3">
          ...
          {pending.length === 0 && !loading && (
            <div className="text-sm text-muted-foreground/70 italic py-8">
          ...
          {pending.map((s) => (
            <div
              key={s.id}
              className="glass rounded-2xl p-6 flex flex-col md:flex-row gap-5 items-start"
            >
          ...
          <div className="mt-16">
            ...
            <div className="space-y-2">
              {reviewed.slice(0, 20).map((s) => (
                <div
                  key={s.id}
                  className="flex items-center justify-between rounded-xl border border-border/30 px-4 py-3 text-sm"
                >
                  <span>
                  ...
                  <span
                    className={`text-xs uppercase tracking-wider ${
                      s.status === "approved" ? "text-emerald-400" : "text-muted-foreground/60"
                    }`}
                  >
                    {s.status}
                  </span>
