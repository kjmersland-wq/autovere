...
export const CarCard = ({ car }: { car: Car }) => {
  ...
  const ui = getUiCopy(i18n.language);
    ...
    >
      <div className="relative aspect-[16/11] overflow-hidden">
        ...
        <div className="absolute top-4 right-4 glass rounded-2xl px-3 py-2 text-right">
          <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{ui.carCardMatch}</div>
      ...
      <div className="p-6">
        ...
        <div className="text-xs text-accent inline-flex items-center gap-1 group-hover:gap-2 transition-all">
          {interpolate(ui.carCardExplore, { name: car.name })} <ArrowRight className="w-3 h-3" />
        </div>
      </div>
    </Link>
  );
