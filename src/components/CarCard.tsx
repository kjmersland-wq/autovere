    ...
    >
      <div className="relative aspect-[16/11] overflow-hidden">
        ...
        <div className="absolute top-4 right-4 glass rounded-2xl px-3 py-2 text-right">
          <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{t("pages.car.match")}</div>
          <div className="text-xl font-bold text-gradient leading-none">{car.score}</div>
        </div>
        <div className="absolute bottom-4 left-5 right-5">
          <div className="text-xs text-muted-foreground mb-1">{l(car.type)}</div>
          <h3 className="text-2xl font-semibold tracking-tight">{car.name}</h3>
        </div>
      </div>
      <div className="p-6">
        <div className="font-medium mb-2 text-foreground">{fit}</div>
        <p className="text-sm text-muted-foreground leading-relaxed mb-4">{l(car.tagline)}</p>
        <div className="text-xs text-accent inline-flex items-center gap-1 group-hover:gap-2 transition-all">
          {t("pages.car.explore", { name: car.name })} <ArrowRight className="w-3 h-3" />
        </div>
      </div>
    </Link>
  );
